...
def autogluon_tabular_training_pipeline(
    ...
    **Pipeline Stages:**
    ...
    1. **Data Loading & Splitting**: Loads tabular (CSV) data from an S3-compatible
       ...
       for "balanced", and up to 10 GiB for "deep"), then performs a two-stage split:
    ...
    **AutoGluon Ensembling Approach:**
    ...
    Args:
        ...
        preset: Training quality tier. "speed" (45-minute selection budget, default, 4 vCPU / 16 GiB), "balanced" (180-minute selection budget, 8 vCPU / 32 GiB), or "deep" (six-hour selection budget, 16 vCPU / 64 GiB).
    ...
    with dsl.Elif(preset == "deep"):
        training_task_deep = autogluon_models_training(**_training_kwargs)
        training_task_deep.set_caching_options(False)
        training_task_deep.set_cpu_request("16").set_memory_request("64Gi").set_cpu_limit("32").set_memory_limit(
            "128Gi"
        )
