...
class TestAutogluonTabularPipelineResourceRequirements:
    ...
    def test_preset_branches_declare_ordered_training_tiers(self):
        """Speed, balanced, and deep branches declare the intended resources."""
        ...
        assert TRAINING_DEEP_RESOURCES in training_resources
        speed = TRAINING_SPEED_RESOURCES
        balanced = TRAINING_BALANCED_RESOURCES
        deep = TRAINING_DEEP_RESOURCES
        assert float(speed.cpu_request) < float(balanced.cpu_request) < float(deep.cpu_request)
        assert int(speed.memory_request.removesuffix("Gi")) < int(balanced.memory_request.removesuffix("Gi"))
        assert int(balanced.memory_request.removesuffix("Gi")) < int(deep.memory_request.removesuffix("Gi"))
