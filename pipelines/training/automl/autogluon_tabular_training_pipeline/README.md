...
| `test_data_secret_name` | `str` | `""` | Optional Kubernetes secret name with S3 credentials for the user-provided test dataset (`TEST_DATA_AWS_*` environment variables). When empty, test data is read with `train_data_secret_name` credentials. |
...
| `test_data_file_key` | `str` | `""` | Optional S3 object key of the test CSV file (features and target column). If provided, `test_data_bucket_name` must also be specified. The test CSV must carry the same feature columns and label column as the training data; a mismatch fails the data loader before training starts. |
