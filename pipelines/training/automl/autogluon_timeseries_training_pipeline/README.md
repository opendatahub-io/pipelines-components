...
1. **Data loading & splitting** (``timeseries_data_loader``): Loads CSV from S3 (up to 100 MiB for the "speed" preset, up to 1 GiB for "balanced", and up to 10 GiB for "deep"), replaces ``+/-inf`` with NaN (missing targets stay for AutoGluon), requires parseable timestamps and non-null ids (or
...
| `preset` | `str` | `speed` | Training quality tier. ``"speed"`` (default, 4 vCPU / 16 GiB), ``"balanced"`` (8 vCPU / 32 GiB), or ``"deep"`` (six-hour budget, 16 vCPU / 64 GiB). |
