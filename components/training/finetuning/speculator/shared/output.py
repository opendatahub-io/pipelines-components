...
def persist_model(
    ...
    Args:
        ...
        log: Logger instance.
        prefer_best: Prefer the checkpoint_best directory when available.
        strict_output: Require an output model artifact.
    """
    preferred = os.path.join(ckpt_dir, "checkpoint_best")
    latest = preferred if prefer_best and os.path.isdir(preferred) else find_model_dir(ckpt_dir)
    if not latest:
        raise RuntimeError(f"No model found in {ckpt_dir}")
    if strict_output and output_model is None:
        raise RuntimeError("Model output artifact is required")
