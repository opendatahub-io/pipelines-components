...
Loads tabular (CSV) data from S3 in batches, sampling up to a preset-dependent size budget (``"speed"``: 100 MB, ``"balanced"``: 1 GB, ``"large_tabular"``: 10 GB), then splits the sampled data into test, selection-train, and extra-train sets.
...
| `preset` | `str` | `speed` | Training quality tier controlling the sampling size budget. ``"speed"`` (default) samples up to 100 MB; ``"balanced"`` samples up to 1 GB; and ``"large_tabular"`` samples up to 10 GB. The cap for user-provided test datasets (50 MB) is unaffected by this setting. |
