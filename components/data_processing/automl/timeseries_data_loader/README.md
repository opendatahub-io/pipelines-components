...
This component loads time series data from S3, samples it (up to 100 MB for the ``"speed"`` preset, up to 1 GB for ``"balanced"``, and up to 10 GB for ``"deep"``), applies light **cleansing** (replace ``+/-inf`` with NaN so AutoGluon can apply its own missing-value logic; require parseable
...
| `preset` | `str` | `speed` | Training quality tier controlling the sampling size budget. ``"speed"`` (default) samples up to 100 MB; ``"balanced"`` samples up to 1 GB; and ``"deep"`` samples up to 10 GB. User-provided test datasets are capped at 50 MB, 100 MB, and 1 GB respectively. |
