...
This component loads time series data from S3, samples it (up to 100 MB for the ``"speed"`` preset, up to 1 GB for ``"balanced"``, and up to 10 GB for ``"heavy"``), applies light **cleansing** (replace ``+/-inf`` with NaN so AutoGluon can apply its own missing-value logic; require parseable
