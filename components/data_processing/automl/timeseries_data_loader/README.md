...
| `test_data_file_key` | `str` | `""` | S3 object key for user-provided test dataset (default: empty string). Must be set together with ``test_data_bucket_name``. When set, the primary temporal split is skipped and this dataset is written to ``sampled_test_dataset``. User-provided test data is capped at 50 MB; larger files are truncated to leading rows. |
